import { AVAILABLE_FEATURES, DEFAULT_PLAN_FEATURES } from '@merchconnect/types';
export class FeatureFlagService {
    ...
    async addFeatureToPlan(planId, featureId) {
    ...
    async createFeatureOverride(override) {
        const newOverride = {
            ...override,
    ...
    async updateFeatureOverride(orgId, overrideId, updates) {
    ...
    async removeFeatureOverride(orgId, overrideId) {
        const orgOverrides = this.overrides.get(orgId) || [];
        const filtered = orgOverrides.filter(o => o.id !== overrideId);
        if (filtered.length === orgOverrides.length)
            return false;
        this.overrides.set(orgId, filtered);
        return true;
    }
    // Feature Check Methods
    async hasFeature(context, featureId) {
        const feature = this.features.get(featureId);
    ...
    async getAvailableFeatures(context) {
        const plan = this.planFeatures.get(context.plan);
        if (!plan)
            return [];
    ...
    // Helper Methods
    getOrganizationOverride(orgId, featureId) {
        const orgOverrides = this.overrides.get(orgId) || [];
        return orgOverrides.find(o => o.featureId === featureId) || null;
    }
    checkCustomRules(rules, context) {
        if (rules.orgId && !rules.orgId.includes(context.orgId))
            return false;
        if (rules.stage && !rules.stage.includes(context.stage))
            return false;
        if (rules.conditions) {
    ...
    getAlternativeFeatures(featureId, currentPlan) {
    ...
    async getOrganizationOverrides(orgId) {
