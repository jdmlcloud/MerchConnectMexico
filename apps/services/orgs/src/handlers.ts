...
export const createOrg: APIGatewayProxyHandlerV2 = async (event) => {
  const body = event.body ? JSON.parse(event.body) : {};
  const parsed = CreateOrgSchema.safeParse(body);
  if (!parsed.success) return { statusCode: 400, body: JSON.stringify({ error: parsed.error.message }) };
  const ddb = getDdb();
  const item = {
...
export const listOrgs: APIGatewayProxyHandlerV2 = async () => {
  const ddb = getDdb();
